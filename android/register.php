<?php
	...
	if ((empty($username))) { 
		...
		if (!empty($username) && $password == $confirm_password){
			$num_rows = mysql_num_rows(mysql_query("SELECT * FROM useras WHERE username='".$username."'"));

			if ($num_rows == 0){
				$query = mysql_query("INSERT INTO useras (id, username, password) VALUES(0,'".$username."','".$password."')");
